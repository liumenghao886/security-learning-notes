...
| vmrun 命令行启动失败 | VMware Authorization Service 通信问题 | 改用 GUI 启动 |
| 扫描扫到一堆 filtered 端口 | 扫的是 **VMware 网关（.254）**，不是靶机 | 先确认自己的 IP，再用 `arp-scan` 找真实靶机 |
| 全端口扫描跑了 22 分钟 | 对 filtered 主机用 `-p-`，每个端口都在等超时 | 先扫常用端口，加 `-T4` 提速 |
